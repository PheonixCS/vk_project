...
# TODO refactor, too similar to discover_movies function
def discover_new_movies(release_date):
    log.debug('discover_new_movies called')

    release_year = release_date.split
    min_average_rating = 6.5
    ...
    if not total_pages:
        log.error('total_pages is empty')
        return
    log.debug(f'got {total_pages} total pages')
    ...
    for page_number in range(1, total_pages):
        ...
        for movie in movies:
            ...
            yield {
                'title': details.get('title', ''),
                'rating': details.get('vote_average', min_average_rating),
                'release_year': datetime.strptime(details.get('release_date', release_year), '%Y-%m-%d').year,
                'country': countries[0] if countries else '',
                'genres': [genre.get('name') for genre in details.get('genres', [])],
                'runtime': details.get('runtime', 120),
                'trailer': choice(trailers),
